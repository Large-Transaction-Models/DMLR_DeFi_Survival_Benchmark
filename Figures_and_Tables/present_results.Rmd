...
```{r}
prediction_results <- read_csv("../Prediction/final_results_original.csv")
cox_results <- read_rds("../Prediction/cox_results.rds") %>%
  select(CIndex)
prediction_results <- prediction_results %>%
  bind_cols(cox_results) %>%
  select(-Cox) %>%
  rename(Cox = CIndex)
rsf_results <- read_rds("../Prediction/rsf_results.rds") %>%
  select(CIndex)

prediction_results <- prediction_results %>%
  bind_cols(rsf_results) %>%
  rename(RSF = CIndex)
...
prediction_results_heatmap <- pheatmap(as.matrix(prediction_results_for_heatmap), scale='none',
         angle_col = 45,
         main = "                                        Survival Prediction Concordance Index Values             C-Index", 
