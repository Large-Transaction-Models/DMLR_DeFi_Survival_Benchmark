...
for (pair in event_pairs) {
  ...
  # Note that we've configured our Cox model to predict survival times rather than hazards rates
...
for (pair in event_pairs){
  ...
  source("~/DMLR_DeFi_Survival_Benchmark/dataLoader.R")
  source("~/DMLR_DeFi_Survival_Benchmark/dataPreprocessing.R")      
  source("~/DMLR_DeFi_Survival_Benchmark/Prediction/random_survival_forest.R") 
  source("~/DMLR_DeFi_Survival_Benchmark/Prediction/concordanceIndex.R") 
  ...
  test <- test_processed %>%
    slice_sample(prop = 0.50)
  # Train the RSF model
  results <- random_survival_forest(train_processed, test_processed)
