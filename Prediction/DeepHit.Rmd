...
for (pair in all_pairs) {
  ...
  final_model <- deep_hit_pipeline(
    ...
    test$timeDiff <- test$timeDiff / 86400  
    train$timeDiff <- train$timeDiff / 86400
    
    # Define time points for IBS calculation
    times <- seq(0, max(test$timeDiff, na.rm = TRUE), length.out = 50)
  
    # Extract event status and survival times
    status_test <- test$status
    time_test <- test$timeDiff
    ...
    predicted_survival_probs <- predict(deephit_model, test, times = times)
...
write.csv(
  results_df,
  file      = "~/KDD_DeFi_Survival_Dataset_And_Benchmark/Survival_Prediction/Results/DeepHit_final_results.csv",
  row.names = FALSE
)
